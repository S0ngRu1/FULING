...
const navItems = [
  { name: "发现角色", path: "/", icon: (p) => `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="${p.isActive ? '#E5B84A' : 'currentColor'}"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>`},
  {
    name: "创建角色",
    path: "#",
    icon: (p) => `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="${p.isActive ? '#E5B84A' : 'currentColor'}" stroke-width="2"><path d="M12 5v14m-7-7h14" /></svg>`
  },
];
</script>
...
<template>
  <div class="h-screen w-screen bg-[#0D0D1A] text-white font-sans flex flex-col overflow-hidden">
    <!-- 全局置顶标题栏 -->
    <header
      class="w-full h-16 bg-[#1A1A2E] flex items-center px-8 shrink-0 z-20 border-b border-white/10">
      ...
      <nav class="w-64 h-full bg-[#1A1A2E] p-4 flex-col shrink-0 hidden sm:flex">
        <div class="flex flex-col space-y-4 mt-4">
          <RouterLink v-for="item in navItems" :key="item.name" :to="item.path"
                      :class="['flex items-center space-x-3 p-3 rounded-lg transition-colors', route.path === item.path ? 'bg-yellow-400/10 text-yellow-300' : 'hover:bg-white/10']">
